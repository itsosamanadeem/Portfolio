...
export default function ProjectContent() {
    ...
    useEffect(() => {
        const getWholeproject = async () => {
            try {
                const res = await fetch(`/api/project/put/${id}`);
        ...
        if (id) getWholeproject();
    }, [id]);

    if (error) return <div className="p-4 text-red-500">project not found.</div>;
